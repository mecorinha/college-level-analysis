...
In order to do this, I first filtered the data so we were only looking at data from 2018 for each school.

```{r}
tuition_income %>% 
  filter(year == "2018") %>% #only looking at 2018 data to make this a little less complicated
  group_by(name) %>% 
  summarise(av_net = mean(net_cost), # create a variable for average net cost (for all income levels)
  ...
  labs(x = "Percent of Tuition Actually Paid", y = "Number of Schools",
       ...
       title = "Percent of Tuition Paid Out of Pocket") +
  scale_x_continuous(labels = scales::percent_format()) # make X axis labels %
```
